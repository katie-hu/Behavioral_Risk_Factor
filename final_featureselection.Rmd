...
```{r}
trainimp <- preProcess(bhv_train, "knnImpute")
...
```{r}
# Final Train and Test
bhv_train <- subset(bhv_train, select = -c(FRUITJU1, HHADULT, NUMWOMEN, NUMMEN, IDAY, DECIDE, VETERAN3, NUMADULT, ADTHINK, ASRCHKUP, ASERVIST, ASDRVIST, HADSIGM3, X_FRUTSUM, HTIN4, PA1MIN_, X_MINAC21))
                    
bhv_test <- subset(bhv_test, select = -c(FRUITJU1, HHADULT, NUMWOMEN, NUMMEN, IDAY, DECIDE, VETERAN3, NUMADULT, ADTHINK, ASRCHKUP, ASERVIST, ASDRVIST, HADSIGM3, X_FRUTSUM, HTIN4, PA1MIN_, X_MINAC21))

trainpr <- subset(trainpr, select = -c(FRUITJU1, HHADULT, NUMWOMEN, NUMMEN, IDAY, DECIDE, VETERAN3, NUMADULT, ADTHINK, ASRCHKUP, ASERVIST, ASDRVIST, HADSIGM3, X_FRUTSUM, HTIN4, PA1MIN_, X_MINAC21))

testpr <- subset(testpr, select = -c(FRUITJU1, HHADULT, NUMWOMEN, NUMMEN, IDAY, DECIDE, VETERAN3, NUMADULT, ADTHINK, ASRCHKUP, ASERVIST, ASDRVIST, HADSIGM3, X_FRUTSUM, HTIN4, PA1MIN_, X_MINAC21))
```
...
set.seed(100)
nnetTune <- train(x = trainpr[,2:170],
                  y = trainpr$GENHLTH,
                  method = "nnet",
                  tuneGrid = nnetGrid,
                  trControl = ctrl,
                  ...
                  linout = TRUE,
                  trace = FALSE,
                  MaxNWts = 13 * (ncol(trainpr[,2:170]) + 1) + 13 + 1,
                  maxit = 1000)

varImp(nnetTune, scale = FALSE)
```
...
knnTune <- train(x = trainpr[,2:170],
                 y = trainpr$GENHLTH,
                 method = "knn",
                 preProc = c("center", "scale"),
                 trControl = ctrl)
varImp(knnTune, scale = FALSE)
```

```{r NSC}
set.seed(476)
nsc <- train(x = trainpr[,2:170], 
                y = trainpr$GENHLTH,
                method = "pam",
                preProc = c("center", "scale"),
